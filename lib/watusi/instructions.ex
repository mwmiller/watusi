defmodule Watusi.Instructions do
  @moduledoc false
  ...
  # Generate opcode/1 function clauses at compile time for all instructions
  for {name, opcode} <- @opcodes do
    def opcode(unquote(name)), do: unquote(opcode)
  end

  def opcode(name), do: raise("Unknown instruction: #{name}")
  ...
  @valtypes %{
    ...
    "i8" => 0x78,
    "i16" => 0x77,
    # Abstract heap types (used as ref.null immediates and reftype shorthands)
    "any" => 0x6E,
    "eq" => 0x6D,
    "struct" => 0x6B,
    "array" => 0x6A,
  ...
  def valtype({:keyword, name}), do: valtype(name)
  def valtype([{:keyword, "ref"}, _] = t), do: {:ref, t}
  def valtype([{:keyword, "ref"}, {:keyword, "null"}, _] = t), do: {:ref, t}

  # Generate valtype/1 function clauses at compile time
  for {name, code} <- @valtypes do
    def valtype(unquote(name)), do: unquote(code)
  end

  def valtype(name), do: raise("Unknown valtype: #{name}")
  ...
  @instr_meta %{
    ...
    "table.get" => {[:i32], [:any]},
    "table.set" => {[:i32, :any], []},
    "table.grow" => {[:any, :i32], [:i32]},
    "table.size" => {[], [:i32]},
    "table.fill" => {[:i32, :any, :i32], []},
    "call" => {[], []},
    "return" => {[], []},
    "throw" => {[:any], []},
    "rethrow" => {[:i32], []},
    ...
    "i16x8.relaxed_q15mulr_s" => {[:v128, :v128], [:v128]},
    "f32x4.relaxed_madd" => {[:v128, :v128, :v128], [:v128]},
    "f32x4.relaxed_nmadd" => {[:v128, :v128, :v128], [:v128]},
    "f64x2.relaxed_madd" => {[:v128, :v128, :v128], [:v128]},
    "f64x2.relaxed_nmadd" => {[:v128, :v128, :v128], [:v128]},
    "i8x16.relaxed_laneselect" => {[:v128, :v128, :v128], [:v128]},
    "i16x8.relaxed_laneselect" => {[:v128, :v128, :v128], [:v128]},
    "i32x4.relaxed_laneselect" => {[:v128, :v128, :v128], [:v128]},
    "i64x2.relaxed_laneselect" => {[:v128, :v128, :v128], [:v128]},
    ...
    "i16x8.relaxed_dot_i8x16_i7x16_s" => {[:v128, :v128], [:v128]},
