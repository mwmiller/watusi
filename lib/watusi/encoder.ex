defmodule Watusi.Encoder do
  ...
  def encode([[{:keyword, "module"} | body]]) do
    ...
    IO.iodata_to_binary([
      ...
      encode_section(2, encode_vector(sections.imports, &encode_import(&1, signatures, sections.types))),
      encode_section(
        3,
        encode_vector(sections.funcs, fn f ->
  ...
  defp passive_data?([{:keyword, "data"} | rest]) do
    ...
    !Enum.any?(rest, fn [{:keyword, _} | _] -> true; _ -> false end)
  end
  ...
  defp encode_export_section(exports, sections, _sigs) do
    encode_section(
      ...
      encode_vector(exports, fn {:export, name, kind, index_or_id} ->
        ...
        kind_byte = case kind do :func -> 0x00; :table -> 0x01; :memory -> 0x02; :global -> 0x03 end
  ...
  defp collect_exports(body, counts) do
    {exports, _, _, _, _} =
      Enum.reduce(body, {[], 0, 0, 0, 0}, fn
        ...
        [{:keyword, "export"}, {:string, name}, [{:keyword, kind}, index_or_id]], {acc, f_i, t_i, m_i, g_i} ->
  ...
  defp collect_shorthands(item, kind, index) do
    item
    |> Enum.filter(fn [{:keyword, "export"}, {:string, _}] -> true; _ -> false end)
  ...
  defp encode_import([{:keyword, "import"}, {:string, mod}, {:string, name}, [{:keyword, "func"} | rest]], signatures, types) do
  ...
  defp encode_import([{:keyword, "import"}, {:string, mod}, {:string, name}, [{:keyword, "table"} | rest]], _, _) do
    [encode_string(mod), encode_string(name), 0x01, 0x70, encode_limits(rest)]
  end
  defp encode_import([{:keyword, "import"}, {:string, mod}, {:string, name}, [{:keyword, "memory"} | rest]], _, _) do
    [encode_string(mod), encode_string(name), 0x02, encode_limits(rest)]
  end
  defp encode_import([{:keyword, "import"}, {:string, mod}, {:string, name}, [{:keyword, "global"}, type_desc]], _, _) do
  ...
  defp encode_table([{:keyword, "table"} | rest]) do
    rest = Enum.reject(rest, fn {:id, _} -> true; [{:keyword, "export"}, _] -> true; _ -> false end)
  ...
  defp encode_limits(tokens) do
    tokens = Enum.filter(tokens, fn {:int, _} -> true; _ -> false end)
    case tokens do
  ...
  defp encode_global([{:keyword, "global"} | rest], ctx) do
    rest = case rest do [{:id, _} | tail] -> tail; _ -> rest end
  ...
  defp encode_elem([{:keyword, "elem"} | rest], ctx) do
    {offset_instrs, rest} = case rest do
    ...
    func_indices = Enum.map(rest, fn
      {:id, _} = id -> resolve_func_index(id, ctx.funcs, ctx.imports)
      {:int, i} -> i
    end)
    [0x00, Enum.map(offset_instrs, &encode_instruction(&1, ctx, [])), 0x0B, encode_vector(func_indices, &encode_u32/1)]
  end

  defp encode_data([{:keyword, "data"} | rest], ctx) do
    offset_expr_item = Enum.find(rest, fn [{:keyword, _} | _] -> true; _ -> false end)
    string = Enum.find(rest, fn {:string, _} -> true; _ -> false end) |> elem(1)
    case offset_expr_item do
      nil -> [0x01, encode_string(string)]
      _ ->
        {memidx, _} = case rest do [{:int, i} | _] -> {i, rest}; _ -> {0, rest} end
        offset_expr = offset_expr_item |> collect_instructions() |> Enum.map(&encode_instruction(&1, ctx, []))
  ...
  defp extract_signature([{:keyword, "func"} | rest] = func, types) do
    case Enum.find(rest, &match?([{:keyword, "type"}, _], &1)) do
      [{:keyword, "type"}, {:id, id}] ->
        type_item = Enum.find(types, &match?([{:keyword, "type"}, {:id, ^id} | _], &1)) || raise("Type not found: $#{id}")
        extract_raw_signature(type_item)
      [{:keyword, "type"}, {:int, i}] -> extract_raw_signature(Enum.at(types, i))
      _ -> extract_raw_signature(func)
    end
  end
  ...
  defp extract_raw_signature([{:keyword, "func"} | rest]) do
    rest = case rest do [{:id, _} | tail] -> tail; _ -> rest end
    params = Enum.flat_map(rest, fn [{:keyword, "param"} | ts] -> Enum.reject(ts, &match?({:id, _}, &1)) |> Enum.map(fn {:keyword, t} -> t end); _ -> [] end)
    results = Enum.flat_map(rest, fn [{:keyword, "result"} | ts] -> Enum.map(ts, fn {:keyword, t} -> t end); _ -> [] end)
    {params, results}
  end
  ...
  defp encode_func_body([{:keyword, "func"} | rest] = func, ctx) do
    ...
    locals = Enum.flat_map(rest, fn [{:keyword, "local"} | ts] -> Enum.reject(ts, &match?({:id, _}, &1)) |> Enum.map(fn {:keyword, t} -> t end); _ -> [] end)
    instructions = collect_instructions(rest)
    func_ctx = %{ctx | local_map: local_map}
    encoded_instrs = Enum.map(instructions, &encode_instruction(&1, func_ctx, []))
    payload = [encode_vector(locals, fn t -> [encode_u32(1), encode_valtype(t)] end), encoded_instrs, 0x0B]
  ...
  defp collect_instructions(rest, label_stack \\ []) do
    rest = case rest do [{:id, _} | tail] -> tail; _ -> rest end
    rest
    |> Enum.reject(fn [{:keyword, "export"}, _] -> true; [{:keyword, k} | _] when k in ["param", "result", "local"] -> true; _ -> false end)
    |> do_collect_instructions([], label_stack)
  end

  defp do_collect_instructions([], acc, _labels), do: Enum.reverse(acc)
  defp do_collect_instructions([[{:keyword, name} | _] | rest], acc, labels) when is_metadata(name), do: do_collect_instructions(rest, acc, labels)
  ...
  defp do_collect_instructions([[{:keyword, name} | args] | rest], acc, labels) when is_control_flow(name) do
    {label, args} = case args do [{:id, id} | tail] -> {id, tail}; _ -> {nil, args} end
    new_labels = [label | labels]
    blocktype = case Enum.find(args, fn [{:keyword, "result"} | _] -> true; _ -> false end) do [{:keyword, "result"}, {:keyword, type}] -> encode_valtype(type); _ -> 0x40 end
    inner_rest = Enum.reject(args, fn [{:keyword, "result"} | _] -> true; _ -> false end)
    {inner_instrs, cond_instrs} = case name do "if" -> {collect_if_instrs(inner_rest, new_labels), collect_if_condition(inner_rest, labels)}; _ -> {collect_instructions(inner_rest, new_labels), []} end
    new_acc = [{:instr, "end", [], labels}] ++ Enum.reverse(inner_instrs) ++ [{:instr, name, [{:int, blocktype}], labels}] ++ Enum.reverse(cond_instrs) ++ acc
    do_collect_instructions(rest, new_acc, labels)
  end
  defp do_collect_instructions([[{:keyword, name} | args] | rest], acc, labels) do
    new_acc = [{:instr, name, filter_immediates(args), labels} | Enum.reverse(collect_folded_args(args, labels))] ++ acc
    do_collect_instructions(rest, new_acc, labels)
  end
  defp do_collect_instructions([_ | rest], acc, labels), do: do_collect_instructions(rest, acc, labels)

  defp collect_if_condition(inner_rest, labels), do: Enum.reject(inner_rest, fn [{:keyword, k} | _] when k in ["then", "else"] -> true; _ -> false end) |> collect_instructions(labels)
  defp collect_if_instrs(inner_rest, labels) do
    then_body = case Enum.find(inner_rest, &match?([{:keyword, "then"} | _], &1)) do [{:keyword, "then"} | body] -> collect_instructions(body, labels); _ -> [] end
    else_body = case Enum.find(inner_rest, &match?([{:keyword, "else"} | _], &1)) do [{:keyword, "else"} | body] -> [{:instr, "else", [], labels} | collect_instructions(body, labels)]; _ -> [] end
    then_body ++ else_body
  end
  defp collect_folded_args(args, labels), do: Enum.filter(args, &match?([{:keyword, _} | _], &1)) |> Enum.flat_map(&collect_instructions([&1], labels))

  defp filter_immediates(args) do
    Enum.filter(args, fn {:int, _} -> true; {:float, _} -> true; {:id, _} -> true; {:offset, _} -> true; {:align, _} -> true; _ -> false end)
  end
  ...
  defp encode_immediates(name, args, ctx, labels) do
    cond do
      ...
      is_bulk_mem(name) -> encode_bulk_mem_immediates(name, args, ctx)
  ...
  defp encode_control_flow_immediates(args) do
    case args do [{:int, bt} | _] -> [bt]; _ -> [0x40] end
  end

  defp encode_bulk_mem_immediates("memory.init", args, ctx) do
    dataidx = case Enum.at(args, 0) do
  ...
  defp encode_bulk_mem_immediates("table.init", args, ctx) do
    elemidx = case Enum.at(args, 0) do
      {:id, id} -> resolve_elem_index(id, ctx.elems)
      {:int, i} -> i
    end
    tableidx = case Enum.at(args, 1) do
  ...
  defp encode_bulk_mem_immediates("table.copy", args, ctx) do
    dst = case Enum.at(args, 0) do
      {:id, id} -> resolve_table_index({:id, id}, ctx.tables, ctx.imports)
      {:int, i} -> i
      nil -> 0
    end
    src = case Enum.at(args, 1) do
      {:id, id} -> resolve_table_index({:id, id}, ctx.tables, ctx.imports)
      {:int, i} -> i
      nil -> 0
    end
  ...
  defp encode_br_table_immediates(args, labels) do
    indices = Enum.map(args, fn {:id, id} -> resolve_label(id, labels); {:int, i} -> i end)
    case indices do [] -> raise "br_table needs target"; _ -> {ts, [d]} = Enum.split(indices, -1); [encode_vector(ts, &encode_u32/1), encode_u32(d)] end
  end

  defp encode_call_indirect_immediates(args, ctx) do
    type_idx = case Enum.find(args, &match?([{:keyword, "type"}, _], &1)) do
  ...
  defp encode_mem_immediates(name, args) do
    offset = case Enum.find(args, &match?({:offset, _}, &1)) do {:offset, v} -> v; _ -> 0 end
    align = case Enum.find(args, &match?({:align, _}, &1)) do {:align, v} -> to_log2(v); _ -> natural_align(name) end
  ...
  defp encode_arg(name, {:id, id}, _, labels) when is_branch(name), do: resolve_label(id, labels) |> encode_u32()
  ...
  defp encode_arg("call", {:id, id}, ctx, _), do: encode_u32(resolve_func_index({:id, id}, ctx.funcs, ctx.imports))
  defp encode_arg(name, {:id, id}, ctx, _) when is_global_op(name), do: encode_u32(resolve_global_index({:id, id}, ctx.globals, ctx.imports))
  ...
  defp resolve_label(id, labels), do: Enum.find_index(labels, &(&1 == id)) || raise("Label not found: $#{id}")
  defp resolve_type_id(id, ctx) do
    idx = Enum.find_index(ctx.types, &match?([{:keyword, "type"}, {:id, ^id} | _], &1)) || raise("Type not found: $#{id}")
  ...
  defp resolve_index(id, list, imports, kind) do
    case find_import_index(imports, kind, id) do
      nil -> count_imports(imports, kind) + (find_local_index(list, kind, id) || raise("ID not found: #{kind} $#{id}"))
      idx -> idx
    end
  end

  defp find_import_index(im, k, id), do: Enum.find_index(im, &match?([{:keyword, "import"}, _, _, [{:keyword, ^k}, {:id, ^id} | _]], &1))
  defp find_local_index(ls, k, id), do: Enum.find_index(ls, &match?([{:keyword, ^k}, {:id, ^id} | _], &1))

  defp resolve_data_index(id, data), do: Enum.find_index(data, &match?([{:keyword, "data"}, {:id, ^id} | _], &1)) || raise("Data ID not found: $#{id}")
  defp resolve_elem_index(id, elems), do: Enum.find_index(elems, &match?([{:keyword, "elem"}, {:id, ^id} | _], &1)) || raise("Elem ID not found: $#{id}")
